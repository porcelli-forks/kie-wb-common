...
public class SessionServiceImpl
    ...
    @Override
    public KieSession newKieSession(final KieModule module,
                                    final String ksessionName) {
        final KieContainer kieContainer = getKieContainerFromKieProject(module);
    ...
    @Override
    public KieSession newDefaultKieSessionWithPseudoClock(final KieModule module) {
        final KieContainer kieContainer = getKieContainerFromKieProject(module);
        //If a KieContainer could not be built there is a build error somewhere; so return null to be handled elsewhere
        if (kieContainer == null) {
            return null;
    ...
    private KieContainer getKieContainerFromKieProject(final KieModule module) {
        return moduleCache.getOrCreateEntry(module).getKieContainer().orElse(null);
    }
}
